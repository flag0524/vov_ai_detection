...
### 전속 모델 · 파일럿 트랙 (2026-07-06 신설, ADR-014)

- C-1. 얼굴 고정 기술검증 — `custom_reference_id` 등록 경로 확인 + `soul/character` 얼굴 고정 실증 (실패 시 MCP 구독+Soul 학습으로 회귀)
- C-2. 후보 모델 4~6장 생성 → 사장님 1명 선정 → canonical reference 등록
- C-3. VOV 레퍼런스 게시물 3~5개 분석 → 프롬프트 템플릿에 스타일 반영 (사용자 링크 제공 대기)
- C-4. 파일럿: 실상품 1~2개 → 화보 4장+릴스 1개 → 육안 GO/NO-GO
- C-5. GO 시 운영 규모 확정 (초기 가이드: 주 2회 게시)

### 착수 순서 권고

```
(현재 확정 순서, ADR-014)  C-1 → C-2 → B-4(검수 UI, ADR-013) → C-3 → C-4 → C-5
(API 공개 후)     B-5
(운영 결정 후)    B-6
```
