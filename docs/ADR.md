...
#### 릴스 비동기화 (진행률 폴링, 2026-07-16) ✅

- **컨텍스트**: `/pipeline/video`가 동기라 릴스 생성(6~10분) 동안 요청이 블로킹되고 진행률이 없었다. Redis는 이 머신에 인프라가 없다(CLAUDE.md).
- **결정**: Redis 대신 **인프로세스 스레드 + job 상태 폴링**. `POST /pipeline/video`는 `GenerationJob(status="queued")`을 만들고 `threading.Thread`로 `_run_reel_job`을 띄운 뒤 **job_id를 즉시 반환**한다. 워커는 자체 `SessionLocal` 세션으로 `queued→running→done/failed`를 갱신한다. 프론트는 `GET /jobs/{id}`를 5초 간격 폴링하며 경과 시간을 표시한다.
- **검증**: 릴스 요청이 0초에 queued 반환 → 3초 뒤 running 확인. 워커 단위 테스트(done/failed 전이) 2종. 63 passed.
- **한계**: 단일 프로세스 스레드라 프로덕션 규모엔 부적합. 다중 워커·재시작 내구성이 필요하면 Redis+Celery로 교체(이 설계가 그 전환의 계약을 미리 맞춰 둠 — job 상태 폴링).
