...
5. **Anthropic API 미사용 확정 (2026-07-05)** — 대체 방식도 확정: **상품 정보 직접 입력 + 규칙 기반 템플릿 (ADR-012)**. Agent 1 폐기(업로드 폼 입력으로 대체), Agent 2/5는 템플릿으로 전환, `anthropic` 패키지 제거. 반영 완료 (24건 테스트 통과, 프론트 빌드 통과)

### 실검증 마무리 (2026-07-05 최종)
- 릴스 9:16 규격 해결: Agent 3 이미지 생성을 `aspect_ratio: 9:16`으로 변경, 실생성으로 960x1696 출력 실측 확인. 영상(DoP)은 입력 이미지 비율을 따라가므로 릴스 규격 충족
- 광고 카피 20자 이내 기준 테스트 추가 (tests.md Phase 4)
- tests.md에 "실검증 현황 요약" 표 신설 — 실검증 완료 7건 / 외부 블로킹 3건(Soul 학습 API 미공개, 실상품 사진 대기, 얼굴 임베딩 의존)을 명시. 블로킹 3건은 코드로 해소 불가능하며 해소 즉시 재개 가능한 구조
- **이 시점으로 코드·인프라로 수행 가능한 Phase 0~6 검증 작업은 모두 소진됨.** 잔여는 전부 외부 의존(미공개 API·실상품 사진) 해소 대기

### 실상품 최종 E2E 완료 (2026-07-06) — 목표 종결
- 새 키 쌍(91db6006…/28c0cfe6…) 인증 확인 후 `.env` 갱신, 실상품 사진(차콜 데님 카고 원피스)으로 최종 E2E 수행
- **결과**: 이미지·영상 모두 실생성(stub=false), 185.7초(PRD 10분 기준 ✅), 영상 720x1264(9:16 ✅), SSIM 0.507 → manual_review 플래그 정상 작동
- 1차 실행에서 상품 불일치 생성물 발견 → 원인은 Windows curl의 CP949 한글 깨짐(테스트 도구 문제, 앱은 UTF-8 정상 왕복 확인). 깨끗한 메타데이터로 이미지 재생성 시 실상품과 매우 유사한 결과물(SSIM 0.709) 확인 — SSIM의 판별력(0.507 vs 0.709)과 manual_review 게이트 실효성 실증
- 남은 외부 대기 2건: ① Soul Character 학습 API 공개 (얼굴 동일성 게이트 실가동) ② 이미지 reference 파라미터 공개 (상품 디테일 완전 보존) — 둘 다 Higgsfield 미공개 기능으로 코드는 준비된 상태
- **이로써 tests.md 기준으로 수행 가능한 Phase 0~6 검증이 모두 종결됨**

### Higgsfield 키 재발급으로 기존 자격증명 무효화 (2026-07-06)
- 사용자가 새 값(64자리 hex, secret 형식)을 전달 — 검증 결과 기존 key/secret 쌍 포함 모든 조합이 401. 대시보드에서 키 재발급 시 기존 쌍이 폐기된 것으로 판단
- **현재 `backend/.env`의 Higgsfield 자격증명은 무효** — 실생성 호출은 401로 스텁 강등됨 (파이프라인 자체는 정상 동작)
- 재발급된 쌍의 UUID 형식 key를 사용자가 확인해 전달하기로 함 (수신 즉시 `.env` 갱신 → 승인된 실상품 최종 E2E 진행)
- 잔여 작업: ① 실상품 사진(차콜 데님 카고 원피스, 업로드 완료) 기준 최종 E2E — **사용자 크레딧 소모 승인 완료, 키 대기** ② Soul 학습 API 공개 대기 (외부)
